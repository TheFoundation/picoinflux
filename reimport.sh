...
test -f $countfile && {
  ...
  for mywinstart in $(seq $start $(cat $importfile|wc -l) )  ;  do
    ...
    echo -ne  "     queue:( $timeranm m $secrem s ) at $tps transactions/s: doing transaction: $mywinstart -> $mywinend  of $importlength ( "$(awk 'BEGIN {print 100*'$mywinstart'/'$importlength'}' |head -c 6 ) " % )  eta $eta  min   "'\r' >&2 ;
