...
(	test -f /proc/loadavg && (cat /proc/loadavg |cut -d" " -f1-3|sed 's/^/load_shortterm=/g;s/ /;load_midterm=/;s/ /;load_longterm=/;s/;/\n/g';)
	...
	which mount >/dev/null && which awk >/dev/null && which df >/dev/null && mount|grep -v docker|grep -e ext4 -e ext3 -e ext2 -e ntfs -e vfat -e reiserfs -e fat32 -e btrfs -e hfsplus -e gluster -e nfs |sed 's/^.\+ on //g'|cut -d" " -f1|while read place ;do df $place  -x devtmpfs -x tmpfs -x debugfs -m |awk '{print $6" "$5}' | column -t|grep "$place"|sed 's/\//-/g;s/^- /root/g;s/^-//g;s/^/diskusepercent_/g;s/%//g;s/ \+/=/g';done
