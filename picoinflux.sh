...
#	echo bloc2 1>&2
  ...
  test /etc/pico.dockerhub.conf && ( for ORGNAME in $(cat /etc/pico.dockerhub.conf |grep -v ^$));do which curl && ( curl -s https://hub.docker.com/v2/repositories/${ORGNAME}/|jq --compact-output '.results  | to_entries[]' |while read imageline ;do echo "$imageline"|jq -c '[.value.namespace,.value.name,.value.pull_count] ' ;done|sed 's/^\["/dockerhub_pullcount,target=/g;s/","/_/g;s/\]//g;s/",/=/g'  );done ) & 
      )
...
grep -q "^SECONDARY=true" $HOME/.picoinflux.conf && (
	...
	( cp $HOME/.influxdata $HOME/.influxdata.secondary )
