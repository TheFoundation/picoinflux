#/bin/bash
if [ "$(id -u)" == 0 ] ;then
which git && ( test -e  /etc/custom/picoinflux/ ||  mkdir -p /etc/custom;cd /etc/custom;git clone https://github.com/TheFoundation/picoinflux.git || ( cd /etc/custom/picoinflux/ ; git pull ) )  
which git || ( test -e  /etc/custom/picoinflux/ ||  mkdir -p /etc/custom/picoinflux/;cd /etc/custom/picoinflux/  ;
